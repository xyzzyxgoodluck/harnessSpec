...
| 触发条件 | 必须写入 |
| --- | --- |
| 首个自动生成物需入库（如 DB 表结构文档、ER 图、API 快照、客户端 SDK） | 生成物文件 + 本表登记一行（含生成命令与时间） |
...
| --- | --- | --- | --- | --- |
| {{db-schema.md}} | {{如 ./mvnw ... 或 scripts/gen-db-schema.ps1}} | {{src/main/resources/db/*.sql + 数据库}} | {{2026-07-01}} | {{只读，勿手改}} |
