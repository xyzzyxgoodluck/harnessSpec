...
- `{{YYYYMMDD}}-{{简述}}.md`：单条 ADR，日期在前、小写 kebab-case，如 `{{20260701-order-id-strategy.md}}`。
...
| 触发条件 | 必须写入 |
| --- | --- |
| 主键策略、缓存一致性、消息幂等/顺序、逻辑删除与唯一索引冲突、是否引入 Flyway、是否启用 MyBatis-Plus 通用 CRUD 抽象（`CrudRepository`）等决策定案 | 一条 ADR + 同步 `../ARCHITECTURE.md` §10 索引 |
| 新增/调整模块、关键链路、中间件拓扑 | 一条 ADR + 同步 `../ARCHITECTURE.md` 对应小节 |
...
| --- | --- | --- | --- | --- | --- |
| {{ADR-001}} | {{订单主键采用雪花 ID}} | {{已接受}} | {{2026-07-01}} | {{ARCHITECTURE §5 / PRD-202607-001}} | `{{20260701-order-id-strategy.md}}` |
